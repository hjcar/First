...
if($_SERVER['REQUEST_METHOD']=='GET' && isset($_GET['start']) && $_GET['start']=='logout'){
 unset($_SESSION['user']);
}
//清除数据库连接数据
...
<body>
  <div class="login">
    <form class="login-wrap" action=" <?php echo $_SERVER['PHP_SELF'] ?> "method="POST"  >
      <img class="avatar" id="avatar" src="/assets/img/default.png">
      <!-- 有错误信息时展示 -->
  ...
  <script>
    ...
    //分析 当邮箱输入完后 失去焦点
    $(function(){
      var emailFname=/^[a-zA-Z0-9_-]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$/;
      $("#email").on("blur",function(){

        if($(this).val()!="" && emailFname.test($(this).val())){
            var value=$(this).val();
            $.get("/admin/api/imgpath.php",{email:value},function(res){
